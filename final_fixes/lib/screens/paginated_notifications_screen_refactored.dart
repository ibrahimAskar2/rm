...
class _PaginatedNotificationsScreenRefactoredState extends State<PaginatedNotificationsScreenRefactored> {
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      ...
      final notifications = notificationsData.map((data) {
        return AppNotification.fromMap(data['id'], data);
      }).toList();
      ...
      setState(() {
        ...
        if (notifications.isNotEmpty) {
          ...
          _lastDocument = notificationsData.last['docSnapshot'] as DocumentSnapshot?;
        }
      });
    } catch (e) {
      setState(() {
    ...
    try {
      ...
      setState(() {
        ...
        if (moreNotifications.isNotEmpty) {
          ...
          _lastDocument = moreNotificationsData.last['docSnapshot'] as DocumentSnapshot?;
        }
      });
    } catch (e) {
      setState(() {
  ...
  // جمع معرفات الإشعارات غير المقروءة
  void _collectUnreadNotificationIds(List<AppNotification> notifications) {
